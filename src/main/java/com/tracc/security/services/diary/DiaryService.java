package com.tracc.security.services.diary;

import com.tracc.exceptions.diary.DiaryEntryNotFoundException;
...
import com.tracc.payload.request.diary.NutritionDiaryEntryRequest;
import com.tracc.payload.request.diary.SleepDiaryEntryRequest;
import com.tracc.payload.request.diary.WorkoutDiaryEntryRequest;
import com.tracc.repository.diary.DiaryEntryRepository;
import com.tracc.repository.diary.DiaryRepository;
import com.tracc.security.jwt.UserProfileUtils;
import com.tracc.models.diary.*;
...
public class DiaryService {
    ...
    public DiaryEntryList getDiaryEntriesByDates(LocalDate start, LocalDate end, String type) {
        String userId = userProfileUtils.getUserDetails().getId();
        Optional<Diary> d = diaryRepository.findByUserId(userId);

        if(!d.isPresent()) {
            throw new DiaryNotFoundException(userId);
        }

        Map<LocalDate, DiaryEntryList> entries = d.get().getEntries();
        DiaryEntryList results = new DiaryEntryList();

        while(start.isBefore(end) || start.isEqual(end)) {
            DiaryEntryList entryForDate = entries.get(start);
            ...
            if(type.equals("ANY")) {
                results.diaryEntries.addAll(entries.get(start).diaryEntries);
            } else {
                results.diaryEntries.addAll(entries.get(start).diaryEntries.stream().filter(entry -> entry != null && entry.getType().equals(type)).collect(Collectors.toList()));
            }
    ...
    /**
     * Retrieve diary entries for a specific date
     * @param type the date to query by
     * @return DiaryEntryList containing entries for that date
     *         if no entries for that date exist return an empty DiaryEntryList
     */
    ...
    public DiaryEntry editEntry(String id, DiaryEntryRequest newDiaryEntry) {
        ...
        diaryEntry.setTimestamp(existing.getTimestamp());
    ...
    private DiaryEntry parseDiaryEntryRequest(DiaryEntryRequest diaryEntryRequest) {
        ...
        else if(diaryEntryRequest instanceof WorkoutDiaryEntryRequest) {
            diaryEntry = new WorkoutDiaryEntry();
            ((WorkoutDiaryEntry) diaryEntry).setExerciseList(((WorkoutDiaryEntryRequest) diaryEntryRequest).getExerciseList());
            if(((WorkoutDiaryEntryRequest) diaryEntryRequest).getName() != null) {
                ((WorkoutDiaryEntry) diaryEntry).setName(((WorkoutDiaryEntryRequest) diaryEntryRequest).getName());
            } else {
                ((WorkoutDiaryEntry) diaryEntry).setName("Free workoutt");
            }
        }
